...
CFG_ROOT_ELEMENT_NAME = "./trackdrives/trackdrive"
...
def init():
    ...
    try:
        ...
        for trackDriveCfg in cfgRoot.findall(CFG_ROOT_ELEMENT_NAME):
            try:
                ...
                settingsNameList = getClassAttributes(TrackDrives[trackDriveID].param.settings)      #['testValue', ...]
                ...
                for settingName in settingsNameList:
                    ...
                    settingCfg = trackDriveCfg.find('.//parameters/settings/setting[@name="' + settingName + '"]')
                    #continue only if configuration exists
                    if settingCfg != None:
                        #convert to float or leave text
                        if is_number(settingCfg.text):              
                            setattr(TrackDrives[trackDriveID].param.settings, settingName, float(settingCfg.text))
                        else:
                            setattr(TrackDrives[trackDriveID].param.settings, settingName, settingCfg.text)

                    print("Setting: {0}".format(TrackDrives[trackDriveID].param.settings.testValue))
